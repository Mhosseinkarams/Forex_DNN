...
class MultiLSTMClassifier:
    """
    A class for multi-class sequence classification using LSTMs.
    ...
    def load_and_preprocess(self, filename="GBPUSD_1h.csv"):
        ...
        for i in range(len(data) - self.num_input_candles):
            ...
            y.append(data.iloc[i + self.num_input_candles]['Multi_Label'])
    ...
    def save_model(self, filename='lstm_multi_classifier.h5'):
        self.model.save(self.base_path / filename)

if __name__ == "__main__":
    classifier = MultiLSTMClassifier()
